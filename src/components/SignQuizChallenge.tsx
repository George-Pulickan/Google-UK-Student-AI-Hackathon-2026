...
export const SignQuizChallenge: React.FC<SignQuizChallengeProps> = ({
  ...
  const currentSign = ASL_SIGNS[currentIndex % ASL_SIGNS.length];

  const handleNextQuestion = () => {
    setCurrentIndex((prev) => (prev + 1) % ASL_SIGNS.length);
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center justify-between">
          ...
          <button
            ...
            className="p-2.5 bg-indigo-50 dark:bg-indigo-950/80 hover:bg-indigo-100 text-indigo-600 dark:text-indigo-300 rounded-xl transition-colors"
