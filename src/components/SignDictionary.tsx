...
import { SignTarget, SignCategory } from '../types';
...
export const SignDictionary: React.FC<SignDictionaryProps> = ({
  ...
  const [activeSign, setActiveSign] = useState<SignTarget>(ASL_SIGNS[0]);

  const filteredSigns = ASL_SIGNS.filter((sign) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            placeholder="Search signs (e.g. A, Hello, Thank You)..."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Right: Active Sign Details Card */}
        <div className="lg:col-span-7">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-xl border border-indigo-100 dark:border-slate-700 space-y-6 sticky top-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-2xl font-bold text-xs shadow-lg shadow-indigo-500/20 transition-all flex items-center gap-2 hover:scale-102"
              >
            ...
            {/* Visual Hint Box */}
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/70 border border-slate-200 dark:border-slate-700 flex items-center gap-3">
              <span className="text-3xl p-2 bg-white dark:bg-slate-800 rounded-xl shadow-xs border border-slate-200 dark:border-slate-700">
                {activeSign.visualHint.split(' ')[0]}
              </span>
              <div>
                ...
                <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                  {activeSign.handShapeDescription}
                </p>
              </div>
            </div>
